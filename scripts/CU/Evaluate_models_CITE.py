# Evaluate all models for CITE data
...
for seed in seeds:     
    models += ['BAVAE_ADT_sample_100_seed_{}'.format(seed)]
    models += ['totalVI_seed_{}'.format(seed)]
    models += ['Winner_CITE_online_seed_{}'.format(seed)]
    models += ['BAVAE_ADT_sample_100_rna_only_seed_{}_20dims'.format(seed)]
    models += ['BAVAE_ADT_sample_100_adt_only_seed_{}_20dims'.format(seed)]
...
for model in models:
    ...
    sc.pl.umap(
        ...
        legend_loc=None,
        wspace=0.0,
        save="_{}_d1_subset_cite.png".format(model)
    )
    
    pd.DataFrame(score_dict).to_csv("Evaluation/scores/CITE/batch_removal_scores_{}.csv".format(model), index=True)
...
sc.pl.umap(
    ...
    ncols=3,
    wspace=1,
    save="_{}_legend_cite.png".format(model)
)
...
sc.pl.umap(
        ...
        legend_loc=None,
        wspace=0.0,
        save="_{}_d1_subset_legend_cite.png".format(model)
)
    

pd.DataFrame(score_dict).to_csv("Evaluation/scores/CITE/batch_removal_scores_all_models.csv", index=True)
